#!/usr/bin/env python3
"""Generate REAL PAMSR comparisons for ALL datasets with polygon annotations.
Datasets: data1_organized, data2_organized, MultiCenter_organized, WBC-Seg
"""
...
DATASETS = {
    "data1": {
        ...
        "img_ext": ".jpg",
        "class_filter": {3, 4, 5, 6},  # Eos, Neu, Lym, Mac
        "max_edge": None,
    },
    "data2": {
        "root": Path("/home/xut/csclip/cell_datasets/data2_organized"),
        "img_ext": ".png",
        "class_filter": {3, 4, 5, 6},
        "max_edge": None,
    },
    "multicenter": {
        "root": Path("/home/xut/csclip/cell_datasets/MultiCenter_organized"),
        "img_ext": ".jpg",
        "class_filter": {3, 4, 5, 6},
        "max_edge": None,
    },
    "wbc": {
        "root": Path("/home/xut/csclip/cell_datasets/WBC Seg/yolo_seg_dataset"),
        "img_ext": ".jpg",
...
def load_gt_with_class(label_path, h, w, class_filter=None):
    masks, cids = [], []
...
def make_panel(img_arr, gt_masks, gt_cids, single_masks, pamsr_masks, info):
    h, w = img_arr.shape[:2]
    p1 = img_arr.copy()
    p2 = draw_mask_outlines(img_arr, single_masks, (0, 200, 200))
    p3 = draw_mask_outlines(img_arr, pamsr_masks, (0, 200, 0))
...
def find_best_candidate(model, ds_cfg, max_scan=6):
    """Scan up to max_scan images and return the one with best PAMSR rescue."""
    import torch
    root = ds_cfg["root"]
    img_dir = root / "images" / "val"
    lbl_dir = root / "labels_polygon" / "val"

    exts = (ds_cfg["img_ext"], ".png", ".jpeg")
    all_imgs = sorted([p for p in img_dir.iterdir() if p.suffix.lower() in exts])
    if not all_imgs:
        return None

    # Shuffle for diversity, but deterministic
    random.seed(42)
    ...
    for idx, ip in enumerate(sampled):
        ...
        try:
            img = load_image(str(ip), max_edge=ds_cfg.get("max_edge"))
        except Exception as e:
            print(f"  SKIP {ip.name}: {e}")
            continue
        h, w = img.shape[:2]
        gt_masks, gt_cids = load_gt_with_class(lp, h, w, class_filter=ds_cfg.get("class_filter"))
        if not gt_masks:
            continue
        ...
        score = stats["rescued"] * 10 + (s_fn - p_fn)
...
def main():
    ...
    results = []
    datasets_to_run = ["data1", "multicenter", "wbc"]
    for ds_name in datasets_to_run:
        print(f"\n{'='*60}")
        print(f"Dataset: {ds_name}")
        print(f"{'='*60}")
        cfg = DATASETS[ds_name]
        cand = find_best_candidate(model, cfg, max_scan=6)
        if cand is None:
            print(f"  No valid candidate found for {ds_name}")
            continue

        info = f"{ds_name} | {cand['stem']} | GT={len(cand['gt_masks'])} rescued={cand['rescued']}"
        panel, rc = make_panel(cand["img"], cand["gt_masks"], cand["gt_cids"],
                               cand["single_masks"], cand["pamsr_masks"], info)
        out_path = OUT_DIR / f"pamsr_real_{ds_name}_{cand['stem']}.png"
        ...
        results.append({
            "dataset": ds_name,
            "stem": cand["stem"],
            "single": (cand["s_tp"], cand["s_fp"], cand["s_fn"]),
            "pamsr": (cand["p_tp"], cand["p_fp"], cand["p_fn"]),
            "rescued": cand["rescued"],
        })

        del cand["img"], cand["gt_masks"], cand["single_masks"], cand["pamsr_masks"]
        gc.collect()
        torch.cuda.empty_cache()

    # Save summary
    with open(OUT_DIR / "pamsr_real_all_datasets_summary.txt", "w") as f:
        for r in results:
            f.write(f"{r['dataset']} / {r['stem']}: Single {r['single']} -> PAMSR {r['pamsr']}  rescued={r['rescued']}\n")
    print(f"\nSaved summary: {OUT_DIR / 'pamsr_real_all_datasets_summary.txt'}")

#!/usr/bin/env python3
"""Fix WBC path and re-scan MultiCenter for better rescued cases."""
...
def process_dataset(model, ds_name, img_dir, lbl_dir, max_scan=8, max_edge=None):
    import torch
    exts = (".jpg", ".jpeg", ".png")
    ...
    for idx, ip in enumerate(sampled):
        ...
        res = model.eval([img], diameter=50, cellprob_threshold=-3.0, channels=[0, 0])
        ...
        # Prefer cases where rescued > 0 AND FN decreased
        score = stats["rescued"] * 20 + max(0, s_fn - p_fn) * 10 - max(0, p_fp - s_fp) * 5
...
def main():
    ...
    # WBC-Seg (fix path to labels/val instead of labels_polygon/val)
    print("\n" + "="*60)
    print("Dataset: wbc")
    print("="*60)
    wbc_root = Path("/home/xut/csclip/cell_datasets/WBC Seg/yolo_seg_dataset")
    wbc_best = process_dataset(model, "wbc", wbc_root / "images" / "val", wbc_root / "labels" / "val",
                               max_scan=8, max_edge=2048)

    # MultiCenter (re-scan for better FN reduction)
    print("\n" + "="*60)
    print("Dataset: multicenter")
    print("="*60)
    mc_root = Path("/home/xut/csclip/cell_datasets/MultiCenter_organized")
    mc_best = process_dataset(model, "multicenter", mc_root / "images" / "val", mc_root / "labels_polygon" / "val",
                              max_scan=10, max_edge=None)

    with open(OUT_DIR / "pamsr_real_fix_wbc_mc.txt", "w") as f:
        for name, best in [("wbc", wbc_best), ("multicenter", mc_best)]:
            if best:
    ...
    print(f"\nSaved: {OUT_DIR / 'pamsr_real_fix_wbc_mc.txt'}")

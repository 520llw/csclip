...
to find a fold where AFP-OD (LW+separation) clearly beats MB-kNN baseline.

V2 fix: use kNN (k=7) as the actual classifier, NOT prototype/NCM.
"""
import sys
import random
import numpy as np
...
OUT_DIR = Path("/home/xut/csclip/paper_materials/best_afpod_fold_v2")
K_KNN = 7
...
def eval_binary_knn_f1(s_i, s_j, q_i, q_j, k=K_KNN):
    """Returns (baseline_f1, lw_f1, afpod_f1) using kNN classifier."""
    X_query = np.vstack([q_j, q_i])
    y_true = np.array([0]*len(q_j) + [1]*len(q_i))
    all_support = np.vstack([s_j, s_i])
    all_labels = np.array([0]*len(s_j) + [1]*len(s_i))

    def _f1(support, labels):
        preds = []
        for q in X_query:
    ...
    base = _f1(all_support, all_labels)

    # AFP-OD with LW
    si_mod, sj_mod = amplify_separation(s_i, s_j, alpha=0.2, method="lw")
    all_support_mod = np.vstack([sj_mod, si_mod])
    afpod = _f1(all_support_mod, all_labels)
...
def main():
    ...
    results = []  # (seed, ci, cj, base_f1, afpod_f1, delta)
    ...
    for seed in SEEDS:
        ...
        for ci, cj in pairs:
            ...
            base, afpod = eval_binary_knn_f1(s_i, s_j, q_i, q_j, k=K_KNN)
            delta = afpod - base
            results.append((seed, ci, cj, base, afpod, delta))
    ...
    seed, ci, cj, base, afpod, delta = best
    print(f"\n{'='*70}")
    print(f"BEST (kNN k={K_KNN}): seed={seed}  {CLASS_NAMES[ci]} vs {CLASS_NAMES[cj]}")
